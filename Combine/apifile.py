...
scan_model = DensePose(model_path="./weights/model_final_162be9.pkl",model_config_path="detectron2/projects/DensePose/configs/densepose_rcnn_R_50_FPN_s1x.yaml")
...
def get_max_shoulder_length(img, r_sho, l_sho):
    return max(compute_each_shoulder_length(img, r_sho), compute_each_shoulder_length(img, l_sho))
...
def process_seg():
    ...
    dolechvai = get_max_shoulder_length(result, result_dict['r_sho'], result_dict['l_sho'])
...
def process_pose():
    ...
    # Chiều dài thân
    midpoint2hip = np.array([int((keypoints[8][0] + keypoints[11][0])/2), int((keypoints[8][1] + keypoints[11][1])/2)])
    ...
    midpoint2sho = np.array([int((keypoints[2][0] + keypoints[5][0])/2), int((keypoints[2][1] + keypoints[5][1])/2)])
    midpoint2ear = np.array([int((keypoints[16][0] + keypoints[17][0])/2), int((keypoints[16][1] + keypoints[17][1])/2)])
    hip_sho_vector = midpoint2sho - midpoint2hip
    sho_ear_vector = midpoint2ear - midpoint2sho
    hunchback_degree = np.dot(hip_sho_vector, sho_ear_vector) / (np.linalg.norm(hip_sho_vector) * np.linalg.norm(sho_ear_vector))
...
def process_data():
    return {"height": int(result_dict["height"]), 
